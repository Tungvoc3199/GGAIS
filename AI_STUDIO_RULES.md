...
4. Tuyệt đối không xóa hoặc thay đổi file `AI_STUDIO_RULES.md`.
...
7. Không tự ý xóa file cấu hình production.
8. Không tự ý đổi Firebase project, Cloud Run service, service account, region hoặc secret name.
9. Không tự ý đổi script deploy-safe trong `package.json`.

## Quy tắc package.json

Script `verify` bắt buộc phải giữ như sau:

```json
"verify": "npm run lint && npm run build && npm run verify:static"
```

Không được đổi `verify` thành:

```json
"verify": "npm run verify:static && npm run verify:security"
```

Lý do: `verify:security` là runtime smoke test, cần token/môi trường runtime thật, không được chạy trong Deploy Cloud Run pipeline.

## Kiểm tra sau khi sửa

Sau khi sửa phải báo rõ:
- Đã sửa file nào
- Lý do sửa
- Logic trước/sau
- Kết quả kiểm tra
