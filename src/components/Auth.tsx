...
export const Auth: React.FC = () => {
  ...
  const handleUiOnlySubmit = (e: React.FormEvent, target: 'trial' | 'activate') => {
    ...
    setInfo(
      target === 'trial'
        ? 'Nhánh UI test: form đăng ký trung tâm đã sẵn sàng giao diện, chưa ghi dữ liệu thật.'
        : 'Nhánh UI test: form kích hoạt tài khoản đã sẵn sàng giao diện, chưa kết nối backend kích hoạt.'
    );
  };
  ...
  const inputBase =
    'w-full rounded-2xl border border-white/10 bg-white/[0.055] px-4 py-3.5 pl-11 text-sm text-white outline-none transition-all placeholder:text-slate-500 focus:border-blue-400/70 focus:bg-white/[0.085] focus:ring-4 focus:ring-blue-500/10';
          ...
          <div className="rounded-[2rem] border border-white/15 bg-white/[0.08] p-6 shadow-2xl shadow-black/30 backdrop-blur-2xl md:p-9">
              ...
              <form className="space-y-5" onSubmit={handleManualLogin}>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    />
                    <Eye className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
                  </div>
                </div>
            ...
            {mode === 'activate' && (
              <form className="space-y-4" onSubmit={(e) => handleUiOnlySubmit(e, 'activate')}>
                {[
                  { label: 'Mã kích hoạt / Link mời', icon: KeyRound },
                  { label: 'Số điện thoại / Email', icon: UserIcon },
                  ...
                  return (
                    <div key={field.label} className="relative">
                      <Icon className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
                      <input type={isPassword ? 'password' : 'text'} placeholder={field.label} className={`${inputBase} ${isPassword ? 'pr-11' : ''}`} />
                      {isPassword && <Eye className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />}
                    </div>
                  );
                })}
